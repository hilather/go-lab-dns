...
func TestFloodAdmissionInflight(t *testing.T) {
	st := LabState("")
	st.Spec.Chaos.Policies[1].Outcomes[0].Actions[0].Duration = 250 * time.Millisecond
	held := NewLab(t, Options{State: st, StartServer: true, MaxInflight: 2, QueryTimeout: time.Second})
	...
	wg.Wait()
	if answers.Load() < 1 {
		t.Fatal("expected some admitted answers")
	}
	if answers.Load() > 6 {
		t.Fatalf("answers=%d", answers.Load())
	}
}

func TestFloodTCPConnectionPressure(t *testing.T) {
	lab := NewLab(t, Options{StartServer: true, MaxTCPConns: 2, MaxTCPPerIP: 2})
	var conns []net.Conn
	defer func() {
		for _, c := range conns {
			_ = c.Close()
