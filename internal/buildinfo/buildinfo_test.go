...
func TestCurrentProtocols(t *testing.T) {
	...
	if info.Protocols.Chaos != ChaosAlgorithm {
		t.Fatalf("Chaos = %q, want %q", info.Protocols.Chaos, ChaosAlgorithm)
	}
	s := info.String()
	if !strings.Contains(s, "labdns") || !strings.Contains(s, MCPProtocol) {
...
func FuzzInfoString(f *testing.F) {
	f.Add("dev", "abc123", "2026-08-15T00:00:00Z")
	f.Add("", "", "")
