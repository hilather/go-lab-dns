...
func TestParityEveryMutatingRESTHasMCPTool(t *testing.T) {
	s, _ := newTestServer(t)
	ts := startHTTP(t, s)
	...
	for _, c := range capabilities.All() {
		...
		if c.MCP == nil || len(c.MCP.Tools) == 0 {
			t.Errorf("%s write without MCP tool", c.ID)
			continue
		}
...
func TestParityGoldens(t *testing.T) {
	...
	compareLines(t, filepath.Join(dir, "prompts.txt"), PromptNames())
...
func TestParityStructuredErrorsMatch(t *testing.T) {
	...
	ts := startHTTP(t, s)
	cs := connectClient(t, ts)
	err := callToolExpectError(t, cs, "dns_zone_get", map[string]any{"zoneId": "nope"})
...
func TestManifestGolden(t *testing.T) {
	...
	if len(doc.Tools) == 0 || len(doc.Resources) == 0 || len(doc.Prompts) != 4 {
