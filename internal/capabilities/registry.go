	...
	for _, c := range all {
		...
		for _, r := range c.MCP.Resources {
			if _, ok := idx.byRes[r]; ok {
				panic("capabilities: duplicate MCP resource " + r)
			}
			idx.byRes[r] = c
		}
	}
...
func init() {
	// Fail closed if a row is internally inconsistent (REST-only with tools, etc.).
...
// LookupTool returns capabilities that expose tool. dns_docs_get is shared
// by the two docs rows.
...
// LookupResource returns the capability bound to a labdns:// URI template.
