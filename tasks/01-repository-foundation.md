...
- **CI jobs later tasks must extend:** format, lint, unit, race, fuzz-smoke, generated-file, documentation, security-scan, container-test. `test-integration`, `test-parity`, `test-config-compat`, and `test-container` are fail-closed Make targets until the owning PRs replace them with real checks. container-test CI currently asserts that fail-closed contract.
