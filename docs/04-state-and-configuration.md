...
- Stable IDs are required for zones, records, forwarding policies, upstreams, and chaos policies.
- IDs are immutable identity within an API version.
...
- Whether stable IDs are user-supplied only or may be generated by planning tools and committed later.
- Whether canonical export preserves comments in a separate sidecar; the initial recommendation is no.
