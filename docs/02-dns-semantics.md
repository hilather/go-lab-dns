...
A matching overlay zone returns a local answer when a local exact or wildcard rule resolves the query. If no local rule resolves it, processing continues to forwarding. Overlay responses are policy overrides, not claims that LabDNS is authoritative for the entire zone.
...
- Configuration compilation rejects detectable CNAME loops.
- Runtime CNAME traversal has a strict configurable depth cap with a safe default.
...
- Set QR on responses.
- Set AA only for authoritative local answers and authoritative negative answers.
- Set RA only when forwarding/recursive service is available to the requesting client.
- Respect RD as a request signal but enforce local policy regardless.
- Do not set AD on synthesized local data.
- Preserve or clear CD according to forwarding behavior and documented DNSSEC policy.
...
- Exact DNSSEC pass-through and validation behavior for the first release.
- Whether overlay CNAME chains may terminate in forwarded names by default or require explicit permission.
