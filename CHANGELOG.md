...
1.1.0 operator-console increment versus `v1.0.0-rc.2`. Candidate notes: [docs/releases/v1.1.0.md](https://github.com/hilather/go-lab-dns/blob/main/docs/releases/v1.1.0.md). This heading stays **Unreleased** until a human tags `v1.1.0`. rc.1/rc.2 notes were not rewritten.
...
- Capability registry `ui` bindings (SPA `route` and `view`/`mutate` `action`) on the existing 31 REST/MCP rows. Disposition is derived from `RESTOnly` (`REST_ONLY_PROTOCOL` vs `PARITY_REQUIRED`); it is not a stored enum.
- REST-only `session` and `ui.assets` catalog rows (`TableRowCount` 33). `POST`/`GET`/`DELETE /v1/session` issue HttpOnly cookie `labdns_session` and CSRF `X-LabDNS-CSRF`. Bearer wins over cookie. Cookie-present `POST /v1/session` without Bearer rotates the existing Actor and does not call Identify. A present but expired/unknown cookie on `POST /v1/session` without Bearer is 401 (does not Identify). SPA GET/HEAD outside `/v1` and `/mcp` is pre-auth and never 401. Session table cap is `rate_limited` / `session table full`.
...
- Playwright operator-matrix e2e (`web/e2e`, `make web-e2e` / `npm run test:e2e`) against loopback `labdns serve` and `testdata/web/` (pack-sample plus viewer vs admin tokens). CI job `web` installs Chromium (`npx playwright install --with-deps chromium`) and runs the matrix. `make web-test` stays Vitest-only. Each operator attempt restores bootstrap (and clears a leftover emergency inhibit) so a CI retry does not plan/apply against a dirty snapshot; failed `web` jobs upload `web/playwright-report` and `web/test-results`.
- Operator onboarding: open `http://127.0.0.1:8080/`, loopback Continue as local administrator or paste a bearer token, `spec.ui.enabled`, and `spec.management.allowedOrigins` in [docs/11-deployment.md](https://github.com/hilather/go-lab-dns/blob/main/docs/11-deployment.md), [docs/12-deployment-repository.md](https://github.com/hilather/go-lab-dns/blob/main/docs/12-deployment-repository.md), [docs/13-operations-and-runbooks.md](https://github.com/hilather/go-lab-dns/blob/main/docs/13-operations-and-runbooks.md), and [examples/labdns-deploy](https://github.com/hilather/go-lab-dns/blob/main/examples/labdns-deploy/README.md). GitOps Compose maps `127.0.0.1:8080:8080`. Hashed `web/dist` is not a `release-diff` surface.
