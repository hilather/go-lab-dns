...
- Operator console login, shell (Overview nav, revision, ready/degraded, empty emergency slot), and status dashboard. `cmd/labdns serve` injects `web.Handler()` so non-loopback `GET /` returns HTML without a bearer while `GET /v1/state` remains 401. CSRF stays in module memory; bearer tokens are discarded after `POST /v1/session`.
